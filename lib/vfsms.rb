...
module Vfsms
  ...
  def self.filter_sms_sent_nos(response,opts)
    ...
    while cycle < send_to_count
      if response.include?("ERROR SEQ=\"#{cycle + 1}\"")
        self.logger.info("Error Occured  : " + opts[:send_to][cycle].to_s)
  ...
  def self.call_api(opts)
    ...
    self.logger.info("SMS Template Id ="+opts[:template].to_s+",  Request Sent : ValueFirst")
    ...
    when Net::HTTPSuccess, Net::HTTPRedirection
      if res.body.include?('GUID')
        self.logger.info("Response : Value First -  " + res.body.split("<MESSAGEACK>")[1].gsub("</MESSAGEACK>",""))
        ...
        self.logger.info("response body missing "+res)
      end
      return false, res.body
    else
      self.logger.info("HTTP Error : #{res}")
